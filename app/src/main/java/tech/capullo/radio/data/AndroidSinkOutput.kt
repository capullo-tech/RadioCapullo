...
class AndroidSinkOutput : SinkOutput {
    ...
    override fun write(buffer: ByteArray, offset: Int, len: Int) {
        when (track!!.write(buffer, offset, len, AudioTrack.WRITE_BLOCKING)) {
            AudioTrack.ERROR ->
                throw IOException("Generic Operation Failure while writing Track")
            AudioTrack.ERROR_BAD_VALUE ->
                throw IOException("Invalid value used while writing Track")
            AudioTrack.ERROR_DEAD_OBJECT ->
                throw IOException("Track Object has died in the meantime")
            AudioTrack.ERROR_INVALID_OPERATION ->
                throw IOException("Failure due to improper use of Track Object methods")
        }
    }
